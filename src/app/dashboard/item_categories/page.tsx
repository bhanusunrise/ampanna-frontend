...
import {
  ...
  updateItemCategory,
  restoreItemCategory, // Import the restore function
} from './functions';
...
import RestoreModal from '@/app/components/Models/restore_model'; // Import the RestoreModal
...
export default function Page() {
  ...
  const [showRestoreModal, setShowRestoreModal] = useState(false); // New state for restore modal
  const [categoryToRestore, setCategoryToRestore] = useState<any>(null); // New state for category to restore
  ...
  const handleRestoreCategory = (rowIndex: number) => {
    const selectedCategoryData = filteredCategories[rowIndex];
    setCategoryToRestore({
      item_category_id: selectedCategoryData[0],
      item_category_name: selectedCategoryData[1],
    });
  ...
  const confirmRestore = async () => {
    if (categoryToRestore) {
      const result = await restoreItemCategory(categoryToRestore.item_category_id);
      if (result.success) {
        await reloadData();
    ...
    <>
      ...
      <Row>
        <Col md={9}>
          <BasicTable
            ...
            table_id="item_categories_table"
            startingIndex={startingIndex}
            onUpdate={handleUpdate}
            onDelete={handleDeleteCategory}
            onRestore={handleRestoreCategory} // Use the restore method
          />
          <NavigateButtons
      ...
      {categoryToRestore && (
        <RestoreModal
          ...
          itemName={categoryToRestore.item_category_name}
        />
      )}
    </>
  );
