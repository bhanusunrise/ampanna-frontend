...
import {
  ACCOUNT_AUTHORIZED, ACCOUNT_EMAIL_LABAL, ACCOUNT_EMAIL_PLACEHOLDER, ACCOUNT_IS_ALLOWED_LABAL,
  ACCOUNT_IS_AN_ADMIN_LABAL, ACCOUNT_IS_MASTER_LABAL, ACCOUNT_IS_NOT_AN_ADMIN_LABAL, ACCOUNT_NAME_LABAL,
  ACCOUNT_NAME_PLACEHOLDER, ACCOUNT_NOT_AUTHORIZED, ACCOUNT_PASSWORD_LABAL, ACCOUNT_PASSWORD_PLACEHOLDER,
  ACCOUNT_PASSWORD_RETYPE_LABAL, ACCOUNT_PASSWORD_RETYPE_PLACEHOLDER, ACCOUNTS_API, ACCOUNTS_PAGE_NAME,
  ACCOUNTS_TABLE_FIELDS, ADD_BUTTON_LABAL, BACK, NEW_ACCOUNT, UPDATE, UPDATE_ACCOUNT_MODEL_TITLE
...
export default function AccountsPage() {
  ...
  // Helper: ensure we always spread a fully typed AccountInterface (not null)
const toDraft = (prev: AccountInterface | null): AccountInterface => ({
  _id: prev?._id ?? "",
  name: prev?.name ?? "",
  email: prev?.email ?? "",            // <-- ensure string, not undefined
  password: prev?.password ?? "",      // optional but safe to default
  retype_password: prev?.retype_password ?? "",
  is_allowed: prev?.is_allowed ?? false,
  is_master: prev?.is_master ?? false,
  token: prev?.token ?? "",
  otp_code: prev?.otp_code ?? 0,
});
  ...
  const callUpdateAPI = async () => {
    try {
      const response = await fetch(`${ACCOUNTS_API}update_account`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(selectedAccountForUpdate),
      });
      ...
      <div className="row">
                ...
                <tr key={index}>
                  ...
                  <td>
                    <Button
                      variant="primary"
                      className="btn-sm"
                      onClick={() => { setSelectedAccountForUpdate(account); setShowUpdateModal(true); }}
                    >
                      🔑
                    </Button>
                  </td>
        ...
        <div className="col-md-4">
          ...
          <TextInput
            form_id="user name"
            onChangeText={(e) =>
              setSelectedAccount(prev => ({ ...toDraft(prev), name: e.target.value }))
            }
            form_message=""
            placeholder_text={ACCOUNT_NAME_PLACEHOLDER}
            label={ACCOUNT_NAME_LABAL}
            value={selectedAccount?.name?? ''}  // ensure string, not undefined
          />

          <TextInput
            form_id="email"
            onChangeText={(e) =>
              setSelectedAccount(prev => ({ ...toDraft(prev), email: e.target.value }))
            }
            form_message=""
            placeholder_text={ACCOUNT_EMAIL_PLACEHOLDER}
            label={ACCOUNT_EMAIL_LABAL}
            value={selectedAccount?.email?? ''}  // ensure string, not undefined
          />

          <PasswordInput
            form_id="password"
            onChangeText={(e) =>
              setSelectedAccount(prev => ({ ...toDraft(prev), password: e.target.value }))
            }
            form_message=""
            label={ACCOUNT_PASSWORD_LABAL}
            value={selectedAccount?.password?? ''}  // ensure string, not undefined
          ...
          <PasswordInput
            ...
            onChangeText={(e) =>
              setSelectedAccount(prev => ({ ...toDraft(prev), retype_password: e.target.value }))
            }
            form_message=""
            label={ACCOUNT_PASSWORD_RETYPE_LABAL}
            value={selectedAccount?.retype_password?? ''}  // ensure string, not undefined
      ...
      {showUpdateModal && selectedAccountForUpdate && (
        <Modal show={showUpdateModal}>
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={() => { callUpdateAPI(); setShowUpdateModal(false); }}>
              {UPDATE}
            </Button>
          </Modal.Footer>
        </Modal>
